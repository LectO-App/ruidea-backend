	...
	try {
		...
		zip
			...
			.then(buffer => {
				...
				blobService.createBlockBlobFromStream('ruidea', blobName, stream, streamLength, error => {
					if (error) {
						console.log(error);
					} else {
						res.send('Subida');
...
router.get('/link-archivos/:id', async (req, res) => {
	try {
		...
		blobService.listBlobsSegmentedWithPrefix('ruidea', ``, undefined, async (error, result) => {
			...
			for await (let file of entries) {
				if (file.name.includes(user.correoElectronico)) {
